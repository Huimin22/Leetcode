...
1. Incremented `res` per node → counted nodes, not depth.
2. No level control → BFS treated as single node loop.
3. Commented out `res += 1` → depth never increased.
4. Didn’t use `size = len(queue)` → couldn’t separate layers.

**Second**

* Forgot that `deque()` requires an iterable
* Confused `deque(root)` with `deque([root])`

**Third**

BFS template: always forgot to add the root to the queue.
...
* Reset `cnt = 0` inside `while` → depth never accumulates.
* Accumulation variables must be defined outside the loop.
* BFS queue `q` updates level by level; `size` controls current layer processing.
