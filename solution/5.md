## 1. Problem
5. Longest Palindromic Substring
    
## 2. Summary

Palindrome = symmetric around a center.

Two centers: single char (odd) or gap (even).

Expand left/right from each center, track the longest.


## 3. Key Points

A palindrome is symmetric, so it can be found by expanding from a center.

**Two types of centers:**

Odd-length → single character

Even-length → gap between two characters


Expand left and right from each center while characters match.

Track longest palindrome with start and end indexes.  

Check all indices as potential centers to cover all palindromes.


## 4. Code Implementation:


```
class Solution:
    def longestPalindrome(self, s: str) -> str:
        start, end = 0, 0
        res = ""
        if len(s) < 1:
            return ""
        
        # Helper function: expand around a given center
        def helper(left, right):
            while left >= 0 and right < len(s) and s[left] == s[right]:
                left -= 1
                right += 1
            return right-left-1
        
        for i in range(len(s)):
            res_max = end - start
            len1 = helper(i, i)
            len2 = helper(i, i+1)
            cur_max = max(len1, len2)
            if cur_max > res_max: 
                end = i + cur_max//2
                start = i - (cur_max-1)//2
        return s[start:end+1]

```

```
class Solution:
    def longestPalindrome(self, s: str) -> str:
        start, end = 0, 0
        res = ""
        if len(s) < 1:
            return ""
        
        # Helper function: expand around a given center
        def helper(left, right):
            while left >= 0 and right <= len(s)-1 and s[left] == s[right]:
                left -= 1
                right += 1
            return right-left-1
        
        for i in range(len(s)):
            res_max = end - start
            len1 = helper(i, i)
            len2 = helper(i, i+1)
            cur_max = max(len1, len2)
            if cur_max > res_max: 
                end = i + cur_max//2
                start = i - (cur_max-1)//2
        return s[start:end+1]

```

## 5. Complexity Analysis
    Time：O(n^2)
        
    Space: O(1)
        
## 6. Edge Cases

## 7. Follow-ups

## 8. Note
**Mistakes:**

Didn’t fully grasp the helper function’s role – it finds the longest palindrome around a center, not just checks if a string is a palindrome.

Confusion about centers – didn’t initially understand:

Odd-length → single character center

Even-length → gap between two characters

Calling the helper twice per index – initially unclear why both odd and even checks are necessary.

Tracking palindrome length – needed clarity on why we calculate right - left - 1 and update start/end.

The idea is not easy to think about. There are two cases when we start from the given center for each char in the str. One case is start from the str[i] ther other is str[i] str[i+1]. This is the key idea about the helper function. By that, we can figure longest substring for each center, that is each char. Then use a for loop to compare each longest substring. 

**Question**
why str[i] str[i+1] instead of str[i-1] str[i], due to symmetry?
Anyway, the helper function is the key. It is not like a common case.

**Second:**

Used wrong boundary in center expansion (left > 0 instead of left >= 0), which truncated valid palindromes.

Misunderstood integer division: // is floor, affecting start / end index calculation.

Not know how to get the end, start well. Need to take some time fto figure it out.

**Third:**

Forgot left >= 0 in the expand loop → invalid expansion

Length formula off by 2 (right - left - 1 needed)

Compared with max_len but never updated it

**Fourth:**

Forgot left >= 0 in the expand loop → invalid expansion

Always forgot the while loop's boundary. I remembered to add right <= len(s)-1 but forgot to 

limit left >= 0.


**Why we need both boundaries**

Left boundary (left >= 0)

If left goes below 0 and you try s[left], Python will throw an IndexError.

Example: s = "abc", left = 0, right = 0 → you enter loop, then left -= 1 → left = -1. Next iteration, s[-1] == s[0]? Python allows negative indices, but now it’s wrapping around to the last character! That’s wrong.

Right boundary (right <= len(s)-1)

Similar logic: if right goes beyond len(s)-1, you try to access a non-existent index → IndexError.

✅ So you must check both: one prevents going out-of-bounds on the left, the other on the right.

**Why we can’t just limit one side**

Palindromes can expand in both directions. Limiting only the right doesn’t protect you from left going negative, and vice versa.

Example: "racecar", center at 'e' (index 3)

Left expands: 2, 1, 0, -1 → stops because left < 0.

Right expands: 4, 5, 6, 7 → stops because right > len(s)-1.

If you only check one side, your expansion either misses some characters or crashes.

**Fifth:**

I added boundary for left and right this time. But the boundary for left is slightly wrong. 

I forgot left can = 0. For convience, I should check like left >= 0 and right <= len(s)-1.

Should use equal way, always be cautious about the boundary especially for equation.


**Sixth:**

**Palindrome length = 4 (even)**
**Center index i = 1 (left-biased center)**

```python
start = i - (4-1)//2 = 1 - 3//2 = 0
end   = i + 4//2   = 1 + 2   = 3
```

✅ Covers indices `0 1 2 3`

---

This shows that the formula

```python
start = i - (len-1)//2
end   = i + len//2
```

works correctly for **even-length palindromes with a left-biased center**.

**Seventh**

Should return right-left-1
