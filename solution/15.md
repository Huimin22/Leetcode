...
I didn’t write the code this way; I had put the while loops elsewhere. 

I didn’t realize that we should skip duplicates immediately after finding a valid triplet, and I didn’t move both pointers after finding the target result.
...
Initialize right outside the for i loop.

Problem: right does not reset for each i.

Fix: move right = len(nums) - 1 inside the loop
...
Works logically (array is sorted, so no sum can be 0 after this).

Usually, people use break here for clarity:
...
Minor bug: used i > 1 for outer-loop deduplication; it should be i > 0. Starting from index 2 causes incorrect results. 

Learned to deduplicate in the inner loop and to move both pointers with boundary checks after finding a valid triplet.

Bug: used () instead of [] when appending a triplet to res.

Also learned that heavy print debugging breaks down on large test cases.

**Fifth Time:**

 while left < right and nums[left] == nums[left+1]:
 
 Not quite sure about why I need to check boundary left < right

 Why check left < right?

Without left < right, this can happen:

Imagine left = right - 1. Then nums[left+1] is nums[right].

You check nums[left] == nums[left+1] → true.

You do left += 1 → now left == right.

If the loop runs again without left < right, nums[left+1] is out of bounds → IndexError.

**So the left < right check prevents going past the right pointer and avoids accessing nums[left+1] when left+1 is outside the array.**

**Sixth Time:**

* Put `return res` inside `for` loop → function exits after first iteration
* Confused `return` (ends function) vs `break` (ends loop)
* Didn’t check loop structure/indentation carefully
