...
Debugged with res and cur_sum initialization. After comparison, update cur_sum, not before. Means we need to decide if 

we to add nums[right] or not after we compare. So update the cur_sum after decision.
...
1. Looping `for right in range(len(nums))` from 0 double-counts `nums[0]`; start from 1.
2. Initializing `cur_m = nums[0]` must match the loop start, otherwise logic is wrong.
3. Using `res = 0` is wrong; for all-negative arrays it gives incorrect result. Use `res = float('-inf')`.
