...
why  while left < right: not  while left <= right:

Cuz finally left = right, that's the res we want to find. So once left = right, stop the loop.

**Fourth**

Pass

Different from lc81.  while left < right. We can't use while left = right, when they are equal, just return the result.
...
The right boundary including mid.
