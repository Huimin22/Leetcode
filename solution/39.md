...
* `start` doesn’t actually change in a single recursion; it’s fixed for that frame.
* Each recursive call gets a new `start` value from the loop variable `i`.
* This is why the DFS explores all combinations without repeating earlier elements unnecessarily.

*Return dfs, it will be null
...
choose from start, not from the beginning.
