...
* Handled empty tree check correctly (`if not root: return res`).
* Forgot to initialize queue with root; wrote `q = deque(); q.append(root)` instead of `q = deque([root])`.
* Misused queue method: wrote `leftpop()` instead of `q.popleft()`.

**Third**

Handled empty tree check correctly (`if not root: return res`). It is common in Tree problem.
