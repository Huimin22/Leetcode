...
Set : right = len(nums) will cause out of range error

Miss corner case:  while left <= right left == right
...
right = len(nums) will cause out of range error

Corner case:  while left <= right left == right [1,1,1] nums[mid] == nums[right] == nums[left]

So when left = right, the loop continues.
...
If we don't need to the following steps and just skip the rest of the code, and go to another loop, use continue.

And need the if statement to handle the corner case, when we are not able to remove the half we don't want.
...
If there is no continue, the mid will be wrong.
